...
async def get_admin_analytics(db: Session = Depends(get_db)):
    ...
    paying_users = db.query(User).filter(User.subscription_plan != "free").count()

    # Запросы по дням за последние 7 дней
    since = datetime.now() - timedelta(days=7)
    queries_by_day = db.query(
        func.date(Query.created_at).label("day"), func.count(Query.id).label("count")
    ).filter(Query.created_at >= since).group_by(func.date(Query.created_at)).all()

    return {
        "active_users": active_users,
        "total_queries": total_queries,
        "paying_users": paying_users,
        "queries_by_day": [{"day": str(day), "count": count} for day, count in queries_by_day]
    }
